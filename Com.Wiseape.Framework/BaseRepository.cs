...
namespace Com.Wiseape.Framework
{
    public abstract class BaseRepository<T> : IRepository<T> where T : class
    {
        ...
        {
            context = (DbContext)Com.Wiseape.Factory.Factory.CreateDataContext<DbContext>(dataContextKey);
        }
        ...
        public virtual List<T> FindAll(SelectParam selectParam, int? page, int? size)
        {
            ...
            else
            {
                if (selectParam != null && selectParam.Where != null)
                    ...
                    items = context.Set<T>().Skip((int)(page - 1) * (int)size).Take((int)size).ToList<T>();
            }
